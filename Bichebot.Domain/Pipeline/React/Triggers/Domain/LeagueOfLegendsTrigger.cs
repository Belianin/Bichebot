    ...
    public class LeagueOfLegendsTrigger : ReactionTrigger
    {
        ...
        public override ReactionReply GetReply(IMessage message)
        {
            return new ReactionReply(core.Random.Choose(
                ...
                $"Вероятность победить в следующей катке: 0%{core.ToEmojiString("dota")}",
                $"Вероятность победить в следующей катке: 100%{core.ToEmojiString("dota")}",
                $"Вероятность победить в следующей катке: 50%{core.ToEmojiString("dota")}",
                ...
                "Ещё одна жертва дедлайна...", $"ПРЕБАФКА К ПЕНСИИ))){core.ToEmojiString("alohabamboe")}",
                ...
                $"{message.Author.Username} умри, насекомое!", $"{message.Author.Username} Не. Хочу. Играть. В говно.",
                $"{message.Author.Username} я бы поиграл с вами, но я просто бот...",
                $"{message.Author.Username} Теперь я вижу всю иронию...",
                $"Всё очень просто. {message.Author.Username} удали Доту.", "Ты спятил",
